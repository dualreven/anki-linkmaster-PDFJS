...
- 以下为必须包含的参考（摘录要点并标注相对路径）：
  ...
  - [`roo/system-prompt-job-executor`](roo/system-prompt-job-executor:1) — 摘录：Job-Executor 的期待输入与输出格式。
  - [`roo/system-prompt-group-job-manager`](roo/system-prompt-group-job-manager:1) — 摘录：Group-Job-Manager 的任务分配与汇总规则。
