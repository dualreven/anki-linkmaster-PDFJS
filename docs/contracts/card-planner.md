...
- `Q`: `annotation-id[]`，卡片正面引用的标注 id 列表（保持顺序，不去重）。
- `A`: `annotation-id[]`，卡片背面引用的标注 id 列表（保持顺序，不去重）。

术语澄清（避免歧义）：
- 本文档中的 `annotation-id` 指 **标注的原始 id 字符串**（例如 `ann_...`），不是包装后的 token。
- 规划器内部如需使用更严格的 token 形态（例如 `[[id]]`）可自行封装，但在“最终制卡输出”边界必须输出原始 id 字符串。

Fail‑Fast：
- `title` 必须为 string；`Q/A` 必须为数组；数组元素必须为 string。
- 建议禁止空任务：`Q` 与 `A` 同时为空数组应视为无意义任务并报错（实现方可在 UI 层阻止或在发射前校验）。
