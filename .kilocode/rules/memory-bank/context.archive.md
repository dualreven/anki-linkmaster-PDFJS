...
### 2025-10-29（新增）— 标注卡片点击“无响应且无日志”的根因与防复发设计
- 问题描述：在 pdf-viewer 的标注侧边栏中，点击卡片或“跳转”按钮无任何反应；查看 `dist/latest/logs/pdf-viewer-*-js.log` 未见相应点击/导航日志。
- 关键根因（工程层面）：
  - 事件作用域不一致：发起侧多处使用 `ScopedEventBus.emit(...)`（模块内事件，形如 `@annotation/...`），而 `AnnotationFeature` 的监听使用 `onGlobal(...)`（全局事件，无 `@` 前缀）。导致“点击触发的跳转事件”无法被监听方接收，表现为无响应。
  - 日志级别屏蔽：`AnnotationFeature.install()` 默认将标注相关模块日志级别设为 `error`（受 `localStorage.ANNOTATION_LOG_LEVEL` 控制，默认 `"error"`），从而 info 级别的点击、路由与处理日志被过滤，造成“看不到日志”的错觉。
  - 委托选择器不统一：`AnnotationSidebarUI` 统一委托监听 `.jump-btn`，但如 `TextHighlightTool` 的卡片跳转按钮并未统一采用该类名（自身会发事件）。当“自身事件又是局部 emit”叠加“监听方 onGlobal”时，功能彻底失效。
- 证据：
  - 监听：`src/frontend/pdf-viewer/features/annotation/index.js` 在 `#setupEventListeners()` 中 `onGlobal(PDF_VIEWER_EVENTS.ANNOTATION.NAVIGATION.JUMP_REQUESTED, ...)`；
  - 触发：工具侧（如 `features/annotation/tools/text-highlight/index.js`）在卡片与按钮 click 使用 `this.#eventBus.emit(PDF_VIEWER_EVENTS.ANNOTATION.NAVIGATION.JUMP_REQUESTED, ...)`；
  - 委托：`features/annotation/components/annotation-sidebar-ui.js` 的 `#setupCardClickDelegation()` 仅匹配 `.jump-btn[data-annotation-id]`；
  - 日志：`dist/latest/logs/pdf-viewer-*-js.log` 有 URL 导航链路的 info 日志，但没有卡片点击相关日志；日志级别覆盖见 `features/annotation/index.js` 的 install 阶段。
- 防复发设计（不改代码的记录；供后续开发遵循）：
  1) 统一跳转通道（服务优先）：提供 `navigationService.navigateToAnnotation(annotationOrId)`，所有发起方直接调用；事件仅用于遥测或跨进程。
  2) 若保留事件：对 `ANNOTATION.NAVIGATION.JUMP_REQUESTED` 强制“全局契约”规范：所有触发必须 `emitGlobal`，监听必须 `onGlobal`。在 `ScopedEventBus` 增加“契约守卫”以防误用（开发模式可抛错）。
  3) 统一 UI 委托选择器：卡片内跳转按钮必须为 `.jump-btn` 且带 `data-annotation-id`，统一走 `AnnotationSidebarUI` 的委托；工具内部的直接 emit 作为兼容保留但不作为主路径。
  4) 契约测试与 E2E：新增“点击卡片应触发导航”的契约测试；并加入“局部 emit 误用应失败/告警”的测试用例。
  5) 调试开关：为 dist 增加 `?debug=annotation` 或启动器注入 `localStorage.ANNOTATION_LOG_LEVEL='info'` 的能力，用于现场快速诊断；生产保持 `error`。
- 验收要点：
  - 任一工具生成的卡片点击均能触发导航，且日志可按需开启查看；
  - URL 导航与卡片点击的优先级清晰且不出现抖动/二次跳转；
  - 事件作用域误用能在开发与测试阶段被立即发现而非静默失败。
...
### 2025-10-29（新增）— gui_launcher + pdfannotation-id 与 page-at 同时存在时的滚动抖动
- 场景：通过 gui_launcher 启动 pdf-viewer，同时传入 `page-at=25` 与 `pdfannotation-id=<某ID>`（生产下映射为 `annotation-id`）。打开后先跳到 25 页，短暂延迟后又回到 22 页（该标注所在页）。
- 代码链路（静态确认）：
  - URL 构造：`src/frontend/pdf-viewer/launcher.py:975-1005` 将 `pdf-id/page-at/position/annotation-id/outline-item-id` 追加到前端 URL。
  - URL 解析与门闸：`features/url-navigation/index.js` 在 `ANNOTATION.DATA.LOADED` 后才执行 `URLJumpDispatcher.tryExecute()`；`components/url-jump-dispatcher.js` 明确优先级 `annotationId > pageAt/position`。
  - 注解跳转：`features/annotation/index.js` 中 `#handleNavigateToAnnotation()` 先 `navigateTo({pageAt})` 再按类型计算 position 居中。
  - 页导航执行：`features/core-navigation/services/navigation-service.js` 触发 `NAVIGATION.GOTO`，由 `UIManagerCore` 设置 `currentPageNumber`。
- 原因推断：
  - 观察到“先 25 后 22”表明存在两条独立的导航触发源按不同时间顺序触发：`page-at` 导航先行，`annotation-id` 导航后至。
  - 依据当前 src 代码，URLNavigationFeature 的“门闸”本应避免在有 `annotation-id` 时执行 `page-at` 分支；但生产构建（dist）可能仍包含较早实现或其他初始化路径在 FILE.LOAD.SUCCESS 后直接执行了 `page-at` 导航，绕过门闸，遂产生二次跳转抖动。
- 验证建议（不改代码）：
  1) 对比 dev 源码模式与 dist 静态模式的 JS 日志（`dist/latest/logs/pdf-viewer-*-js.log`），检索关键词：`[dispatcher] 执行页面导航/annotation 跳转请求`、`NAVIGATION.GOTO`、`FILE.LOAD.SUCCESS`、`ANNOTATION.DATA.LOADED`，核对真正触发顺序。
  2) 仅在 GUI 端临时移除 `page-at`（当 `annotation-id` 存在时）复测，若抖动消失，可印证“双源竞争”结论。
- 修复思路（备忘，后续实现）：
  - 统一约束：当 URL 存在 `annotation-id` 时，任何“初始化阶段的 page 导航”均应被抑制（包括旧路径）；仅放行 annotation 导航。可选实现：
    1) GUI 层：构造 URL 时若有 `annotation-id` 则不拼 `page-at`；
    2) 前端层：在 URLNavigationFeature 安装期设置“annotation 优先”全局旗标，拦截/丢弃早到的 `NAVIGATION.GOTO`（非注解发起）直到 `ANNOTATION.DATA.LOADED`；
    3) 若 dist 与 src 存在版本差异，需统一构建包含上述门闸逻辑。
