...
## 2025-10-07 当前任务（pdf-home 删除按钮无反应）

- 问题描述：在搜索栏中点击“删除选中”（`#batch-delete-btn`）无任何反馈；用户希望先验证按钮点击是否被触发。
- 现状分析：
  - 按钮由 `src/frontend/pdf-home/features/search/components/search-bar.js` 渲染，但未为删除按钮绑定任何监听；
  - `PDFListFeature` 中存在 `#setupButtonListeners()` 试图绑定 `#batch-delete-btn`，但可能因安装时机或异常导致未生效；
  - 为快速验证点击链路，先在 `SearchBar` 本地绑定点击并 toast 提示，再视情况在 Feature 间做事件桥接。
- 相关模块/函数：
  - `SearchBar.render()` / `#attachEventListeners()`
  - `PDFListFeature.#setupButtonListeners()` / `#handleBatchDelete()`
  - Toast 工具：`src/frontend/common/utils/notification.js`

### 计划与原子步骤
1) 增加 `SearchBar` 删除按钮点击监听，点击时 `showInfo('调试：已点击“删除选中”按钮')` 并本地 `eventBus.emit('search:delete:clicked')`；
2) 添加 Jest 单测，验证点击后触发 `search:delete:clicked`；
3) 如仍无后端响应，再核对 `PDFListFeature` 的按钮监听是否注册（安装顺序/时机），必要时在 `SearchFeature` 中桥接到全局事件供 `PDFListFeature` 监听；
4) 端到端联调：点击后应弹确认框，并向 WS 发送 `pdf-library:remove:requested`；

### 执行结果（本次）
- 已实现步骤 1、2：点击删除按钮即弹出调试 toast，且单测覆盖事件触发；
- 未改动架构，仅补齐 UI 监听与测试。

#### 变更补充（2025-10-07 06:08）
- 应用户最新要求：
  - 移除 SearchBar 的“删除选中”按钮（不再渲染/监听）。
  - 将批量删除回调绑定到搜索结果头部的“🗑️ 删除”按钮（`.batch-btn-delete`）。
  - 为兼容其它区域已有按钮，`PDFListFeature` 仍保留对 `#batch-delete-btn` 的监听。
  - 删除与“删除选中”按钮相关的单测文件。

#### 变更补充（2025-10-07 06:16）
- 修复“删除成功但页面不刷新、且无toast”的根因：
  - WSClient 未将 `pdf-library:remove:completed/failed` 路由为 `websocket:message:response`；
  - 已将两类消息映射到 RESPONSE（并结算 pending）；
  - PDFListFeature 既有监听可正确弹出 toast，并调用 `#refreshAfterDeletion()` 触发 `search:query:requested` 更新搜索结果。

#### 变更补充（2025-10-07 06:40）
- 删除/添加一致性与重复添加：
  - 删除：`standard_server.handle_batch_pdf_remove_request` 同步尝试 `pdf_manager.remove_file(fid)`，避免 file_list 残留导致后续添加报“已存在”。
  - 添加：放宽重复校验（幂等）
    - `pdf_manager/standard_manager.add_file`：若已存在，直接返回现有记录，视为成功；
    - `pdf_manager/manager.add_file`：若已存在，触发 file_added/file_list_changed 并返回 True。
  - 目标：删除后可重新添加；对同路径重复“添加”返回成功而非报错。
...
- 名称：实现 pdf-home 的“删除选中 PDF”功能（批量）
- 背景：前端已有列表与搜索模块，用户希望以“选中多条 → 一次删除”的方式操作，删除过程需有 toast 反馈，并在完成后刷新“搜索结果列表”。

### 问题描述
- 选中方式不固定（可能来自表格勾选、其他批量选择器等），但传递给删除插件时必须规整为 pdf-id 数组。
- 发送删除请求时需提示“删除中”；
- 删除成功提示“删除成功”，并刷新 pdf-home 的搜索结果列表；
- 失败提示“删除失败-<失败原因>”。

### 相关模块与函数
- 前端
  - `src/frontend/pdf-home/features/pdf-list/index.js`
    - `#handleBatchDelete()`：收集选中项、toastPending、发送删除请求（`WEBSOCKET_MESSAGE_TYPES.REMOVE_PDF`，`data.file_ids`）。
    - `#refreshAfterDeletion()`：优先依据搜索框 `.search-input` 触发 `search:query:requested`，否则回退到 `GET_PDF_LIST`。
    - WebSocket 响应处理：
      - `pdf-library:remove:completed` → `toastSuccess/toastWarning/toastError` + 增量删除 Tabulator 行 + 清空选中 + 刷新。
      - `pdf-library:remove:failed` → `toastError`。
  - `src/frontend/pdf-home/features/search/components/search-bar.js`
    - 新增按钮 `#batch-delete-btn`（“➖ 删除选中”）。
- 后端
  - `src/backend/msgCenter_server/standard_server.py`
    - `handle_batch_pdf_remove_request()`：接收 `data.file_ids`，返回 `removed_files/failed_files`，类型 `pdf-library:remove:completed/failed`。
  - `src/backend/msgCenter_server/standard_protocol.py`
    - `build_batch_pdf_remove_response()`：标准响应载荷结构。

### 执行步骤（原子化）
1. 读取最近8条工作日志与 SPEC/README，加载 memory bank。
2. 设计消息 Schema 与前端事件流（使用 `WEBSOCKET_MESSAGE_TYPES.REMOVE_PDF`）。
3. 新增前端按钮（search-bar）与批量删除逻辑（pdf-list）。
4. 发送删除请求时使用 `toastPending`；成功/失败使用 `toastSuccess/toastWarning/toastError`。
5. 删除完成后优先刷新搜索结果（触发 `search:query:requested`），否则回退到 `GET_PDF_LIST`。
6. 编写 Jest 测试：验证消息格式与刷新行为。
7. 本地运行测试并联调（如需）。
8. 快速修复：为删除流程增加多源选中ID收集（state/Tabulator/搜索结果复选框/选中条目）以兼容 UI 未初始化或采用新搜索结果列表的情况。

### 已知信息/提醒
- 事件命名统一三段式；WS 请求类型统一使用 `*:requested` 常量；响应解析优先使用 `*:completed/failed`。
- 遵循 UTF-8 与 `\n` 规范；避免直接 `console.*`，统一使用 Logger。
