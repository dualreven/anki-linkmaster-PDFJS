...
## 当前任务（20251010015413）
- 名称：修复 pdf-viewer 大纲拖拽设为子节点后“其他节点被删除”与后端 UNIQUE 冲突
- 问题背景：将根节点1拖拽到节点2成为子节点后，侧栏显示剩余节点消失；后端报错 `bookmark:save:failed`（UNIQUE constraint failed: pdf_bookmark.bookmark_id）。初判为保存载荷包含重复 id 或重复保存竞态。
- 相关模块与函数：
  - 前端 UI：`src/frontend/pdf-viewer/ui/bookmark-sidebar-ui.js`（拖拽处理、REORDER 事件发出）
  - 前端 Feature：`src/frontend/pdf-viewer/features/pdf-bookmark/index.js`（#handleReorderRequest、#refreshBookmarkList）
  - 前端 服务：`src/frontend/pdf-viewer/features/pdf-bookmark/services/bookmark-manager.js`（reorderBookmarks、saveToStorage、loadFromStorage）
  - 前端 存储：`src/frontend/pdf-viewer/features/pdf-bookmark/services/bookmark-storage.js`（Remote/Local 保存载荷结构）
  - 后端 API：`src/backend/api/pdf-viewer/bookmark/service.py`（list_bookmarks/save_bookmarks 树展平/删除重建）
- 执行步骤（原子化）：
  1) 阅读上述前后端代码链路，确认事件与载荷结构
  2) 设计测试：移动根→子节点后保存载荷完整且全树 id 唯一
  3) 修复：
     - BookmarkManager.saveToStorage 对树进行基于 id 的去重序列化（防御重复引用）
     - PDFBookmarkFeature.#handleReorderRequest 移除重复 save 调用，避免竞态
  4) 运行测试与 ESLint 校验
  5) 记录日志并通知完成
- 状态：进行中
