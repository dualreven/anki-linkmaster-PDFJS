...
**注意**：2025年11月的详细变更记录已移至 `context.md`，此处仅保留核心技术规范。历史变更详见 `docs/context-archive/`。
  ...
  - **Feature 内部 EventBus 门禁（增量）**：随 `pnpm run lint` 运行（接入点：`scripts/ci/frontend-line-limit.js`）；基线文件为 `scripts/ci/baselines/feature-internal-eventbus-gates.json`；更新基线：`node scripts/ci/feature-internal-eventbus-gates.js --write-baseline`。
...
- 现行检查清单（最小集合）
  ...
  - 并行合入工具：使用 `scripts/merge-fastlane.ps1` 在 main 侧创建 integration 分支批量 cherry-pick，并一键跑 lint + 指定 Jest 测试输出报告（`--runTestsByPath` 必须传“文件路径”，不要传目录）。
  - 并行协作流程：见 `.kilocode/rules/memory-bank/workflow-parallel-worktrees.md`（worktree 分工、验收合入、冲突规避）。
