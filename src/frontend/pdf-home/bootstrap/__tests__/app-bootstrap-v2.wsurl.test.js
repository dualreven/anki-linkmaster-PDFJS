...
describe("pdf-home bootstrap WS URL (IPv4 loopback)", () => {
  ...
  beforeAll(() => {
    // 显式设置运行时端口，避免依赖 window.location 解析（测试环境已提供 jsdom）
  ...
  it("should construct ws://127.0.0.1:<port> for msg center", async () => {
    // Mock 端口解析，避免命中 import.meta，统一使用 common/utils/ws-port-resolver
    jest.doMock("../../../common/utils/ws-port-resolver.js", () => {
      return {
