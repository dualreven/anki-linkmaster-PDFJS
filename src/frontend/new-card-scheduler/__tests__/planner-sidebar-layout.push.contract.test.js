...
function setupDom() {
  ...
  return { sidebarEl, mainEl, toolbarEl };
}
...
describe("new-card-scheduler sidebar push layout (F) - contract regression", () => {
  ...
  test("安装后默认展开：主区域被推开", () => {
    ...
    expect(mainEl.style.marginLeft).toBe("320px");
    expect(mainEl.style.width).toBe("calc(100% - 320px)");
  ...
  test("点击一次：侧边栏收起，主区域恢复", () => {
    ...
    expect(mainEl.style.marginLeft).toBe("40px");
    expect(mainEl.style.width).toBe("calc(100% - 40px)");
  ...
  test("dispose 后：再点击不再改动布局", () => {
    ...
    expect(document.querySelector("#planner-sidebar-toggle-btn")).toBeFalsy();
