...
def _setup_logging() -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(logs_dir / 'pdf-viewer.log', encoding='utf-8', mode='w'),
...
def main() -> int:
    _setup_logging()
    logger.info("Launching pdf-viewer standalone window")
    ...
    try:
        ...
        bridge = PdfViewerBridge(ws_client, window, args.file_path)
        ...
        def on_connected():
            ...
            if args.file_path:
                bridge.loadPdfFile(args.file_path)
    ...
    def delayed_js_logger_start():
        ...
        try:
            js_console_logger = JSConsoleLogger(
                ...
                log_file=str(project_root / 'logs' / 'pdf-viewer-js.log')
            )
    ...
    # Load front-end page
    url = f"http://localhost:{vite_port}/pdf-viewer/?ws={ws_port}&pdfs={pdf_port}"
    if args.file_path:
        # URL-encode the file path
        import urllib.parse
        file_param = urllib.parse.quote(args.file_path)
