...
/**
 * 目标：验证 SidebarManagerFeature 安装后，可以成功注册并打开若干核心侧边栏（anchor/outline/annotation），
...
describe("SidebarManagerFeature - 注册与打开侧边栏", () => {
  ...
  test("能够打开 anchor / outline / annotation 侧边栏并渲染容器 DOM", async () => {
    ...
    expect(ids).toEqual(expect.arrayContaining(["anchor", "outline", "annotation"]));
  });
});
