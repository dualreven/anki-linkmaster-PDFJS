...
export class SearchEngine {
  ...
  async initialize(pdfViewer, pdfEventBus, pdfLinkService) {
    ...
    // pdfLinkService 当前未在引擎内部使用；不持有以避免未用成员告警
  ...
  #setupPDFJSEventListeners() {
    // 监听搜索结果更新
    this.#pdfEventBus.on(PDF_VIEWER_EVENTS.PDFJS_EVENTS.FIND.UPDATE_MATCHES_COUNT, (event) => {
      this.#handleFindMatchesCount(event);
    });

    // 监听当前匹配项变化
    this.#pdfEventBus.on(PDF_VIEWER_EVENTS.PDFJS_EVENTS.FIND.UPDATE_CONTROL_STATE, (event) => {
