...
export class URLNavigationFeature {
  ...
  async install(context) {
    ...
    if (this.#parsedParams.hasParams) {
      ...
      // 若包含 outlineItemId，立即广播一次 OUTLINE 按ID导航请求（确保订阅者可收到）
      if (this.#parsedParams?.outlineItemId) {
        try {
          this.#eventBus.emitGlobal(
            PDF_VIEWER_EVENTS.OUTLINE.NAVIGATE_BY_ID.REQUESTED,
            { outlineItemId: this.#parsedParams.outlineItemId },
            { actorId: "URLNavigationFeature" }
          );
        } catch {
          this.#eventBus.emit(
      ...
      try {
        ...
        if (pdfId && typeof pdfId === "string" && pdfId.trim().length > 0) {
          ...
          try {
            this.#logger.info("[TRACE] Emitting FILE.LOAD.REQUESTED from URLNavigationFeature", { pdfId, filename: filenameForLoad, source: "url-params" });
          } catch (e) { void e; }
    ...
    // 渲染就绪门闸：首屏渲染完成即可进行 page/anchor/outline 导航
    this.#eventBus.on(PDF_VIEWER_EVENTS.RENDER.READY, (info) => {
      try { this.#logger.info(`[url-navigation] RENDER.READY received, totalPages=${info?.totalPages}`); } catch {}
  ...
  async #handlePDFLoadSuccess() {
    ...
    // 文件加载成功：视作“基本就绪”的门槛；若未收到 RENDER.READY，则以此作为兜底
    if (this.#hasProcessedParams) { return; }
    this.#renderReady = this.#renderReady || true;
    this.#logger.info("[url-navigation] 文件加载成功（兜底视作渲染就绪），尝试执行门闸导航");
    this.#tryExecuteGatedNavigation();
  }

  async #tryExecuteGatedNavigation() {
    if (this.#gatedNavigationDone) { return; }
    if (!this.#parsedParams || !this.#parsedParams.hasParams) { return; }

    const { annotationId } = this.#parsedParams || {};

    // 规则：
    // - annotationId：需要标注数据门闸
    // - 其余（page/anchor/outline）：只需渲染就绪；避免被 annotation 门闸阻塞
    if (annotationId && !this.#annotationDataLoaded) {
      this.#logger.info("[URLNavigationFeature] 等待标注数据就绪以执行 annotation 导航");
      return;
    }
    if (!annotationId) {
      if (!this.#renderReady) {
        this.#logger.info("[URLNavigationFeature] 等待渲染就绪以执行非注解导航");
        return;
      }
    }
    ...
    try {
      ...
      if (res.type === "page") {
        ...
        this.#logger.info("[URLNavigationFeature] outline 导航已触发（广播一次以确保订阅者接收）", res, { toast: { type: "info", ms: 2000 } });
        try {
          this.#eventBus.emitGlobal(
            PDF_VIEWER_EVENTS.OUTLINE.NAVIGATE_BY_ID.REQUESTED,
            { outlineItemId: this.#parsedParams?.outlineItemId || null },
            { actorId: "URLNavigationFeature" }
          );
        } catch {
          this.#eventBus.emit(
