...
export class AnnotationSidebarUI {
  ...
  #createAnnotationCard(annotation) {
    ...
    commentBtn.setAttribute(
      'aria-label',
      commentCount > 0 ? `查看评论（${commentCount}）` : '添加评论'
    );
  ...
  #showCommentDialog(annotationId) {
    ...
    const submitComment = () => {
      ...
      if (!titleChanged && !tagsChanged && !hasComment) {
        // 没有任何变化，显示警告
        this.#logger.warn('没有更改', { toast: { type: 'warn', ms: 3000 } });
        return;
      }

      // 处理标题和标签变化（只负责本地状态与 UI，持久化交给 AnnotationManager）
      ...
      // 处理评论添加（本地先写入 Annotation，再交给 AnnotationManager 统一保存）
      if (hasComment) {
        ...
        // 发出评论事件（用于其它监听者；本地已更新，因此标记 skipUpdate）
      ...
      // 统一通过 UPDATE 请求交给 AnnotationManager 持久化（包括 title 与 comments）
      const changes = {};
      if (titleChanged) {
        changes.title = currentTitle || null;
      }
      // tags 暂为 UI 占位，不写入后端 json_data，未来有需要再扩展 changes.data
