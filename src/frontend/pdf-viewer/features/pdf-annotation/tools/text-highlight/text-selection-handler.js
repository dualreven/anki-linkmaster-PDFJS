...
export class TextSelectionHandler {
  ...
  #handleMouseUp(e) {
    ...
    setTimeout(() => {
      ...
      // 发送文本选择完成事件（使用常量：TEXT_SELECTED）
      import("../../../../../common/event/pdf-viewer-constants.js").then(({ PDF_VIEWER_EVENTS }) => {
        this.#eventBus.emit(PDF_VIEWER_EVENTS.ANNOTATION.TEXT_HIGHLIGHT.TEXT_SELECTED, {
          text: text,
        ...
        this.#logger?.warn?.("[TextSelectionHandler] Failed to emit TEXT_SELECTED due to constants import failure");
      });
    }, 10);
  }
