  ...
  #createCustomColorSection() {
    ...
    applyButton.addEventListener("click", () => {
      ...
      if (/^#[0-9A-Fa-f]{6}$/.test(color)) {
        ...
        // 内联错误提示，替代 alert
        let err = section.querySelector(".color-error");
        if (!err) {
          err = document.createElement("div");
          err.className = "color-error";
          err.style.cssText = "color:#c62828;background:#ffebee;border:1px solid #ffcdd2;padding:6px 10px;border-radius:4px;margin-top:8px;";
          section.appendChild(err);
        }
        err.textContent = "请输入有效的颜色格式（例如：#ffff00）";
      }
    });
