...
describe("PDFUrlLoaderFeature", () => {
  ...
  beforeEach(() => {
    ...
    // 创建 Feature 实例
    feature = new PDFUrlLoaderFeature();
  });
  ...
  describe("Feature基本信息", () => {
    test("应该正确定义Feature名称", () => {
      expect(feature.name).toBe("pdf-url-loader");
    });
  ...
  describe("Feature生命周期", () => {
    ...
    test("应该成功安装Feature（有URL参数）", async () => {
      window.history.pushState({}, "", "http://localhost/?pdf-id=sample&title=Sample");
      await expect(feature.install(container)).resolves.not.toThrow();
      ...
      expect(parsedEvent[1]).toMatchObject({ pdfId: "sample", title: "Sample" });
    });
  ...
  describe("URL参数处理", () => {
    ...
    test("有pdf-id参数时应该触发PDF加载", async () => {
      ...
      expect(loadRequestEvent[1]).toMatchObject({ filename: "sample", source: "pdf-url-loader" });
    });
