...
# -*- coding: utf-8 -*-
"""
Anno/NewCardScheduler/CustomReviewer Launchers

为三个工具窗口复用 SimpleWebWindowApp 提供薄包装，供 Hosted/CLI 启动使用。
"""
...
class AnnoManagerApp(SimpleWebWindowApp):
  """标注管理器窗口启动器。"""

  def __init__(self, config: LaunchConfig, parent_app: Optional[QApplication] = None) -> None:
    super().__init__(config, entry_path="anno-manager", window_title="标注管理器", parent_app=parent_app)
