...
export class ErrorHandler { constructor(eventBus) { this.eventBus = eventBus; this.logger = new Logger("ErrorHandler"); }
  ...
  showUserFriendlyError(error) { let userMessage = "操作失败，请稍后重试"; switch (error.type) { case ErrorType.BUSINESS: userMessage = error.message || "业务逻辑错误"; break; case ErrorType.NETWORK: userMessage = "网络连接失败，请检查网络设置"; break; case ErrorType.SYSTEM: userMessage = "系统错误，请联系管理员"; break; } this.eventBus.emit(UI_EVENTS.ERROR.SHOW, { message: userMessage, type: error.type }); }
