...
export class PDFManager {
  ...
  #handlePDFListUpdate(data, source) { try { this.#logger.info(`Processing PDF list from '${source}': ${JSON.stringify(data, null, 2).substring(0, 100)}[truncated]`); const files = data?.data?.files || []; const newPdfs = files.map(file => this.#mapBackendToFrontend(file)); // Deduplicate updates: skip if identical to current state to avoid racing UI updates try { const areEqual = Array.isArray(this.#pdfs) && this.#pdfs.length === newPdfs.length && this.#pdfs.every((p, idx) => String(p.id) === String(newPdfs[idx].id)); if (areEqual) { this.#logger.debug("Received PDF list identical to current state, skipping update."); return; } } catch(e) { this.#logger.warn("Error comparing pdf lists for deduplication", e); } this.#pdfs = newPdfs; this.#logger.info(`Processed ${this.#pdfs.length} PDF files: ${JSON.stringify(this.#pdfs, null, 2).substring(0, 100)}[truncated]`); this.#eventBus.emit(PDF_MANAGEMENT_EVENTS.LIST.UPDATED, this.#pdfs, { actorId: 'PDFManager' }); } catch (error) { this.#logger.error(`Error processing PDF list update from ${source}: ${error.message}`, error); this.#eventBus.emit(PDF_MANAGEMENT_EVENTS.ERROR.OCCURRED, { message: "Failed to process PDF list." }, { actorId: 'PDFManager' }); } }
  ...
  #handleResponse(data) {
    ...
    try {
      ...
      const batchId = respData?.batch_request_id || respData?.batch_request_id_str || null;
      const originalType = respData?.original_type || data?.original_type || respData?.type || null;

      if (batchId && this.#batchTrack.has(batchId)) {
        const entry = this.#batchTrack.get(batchId);
        // 将成功或失败均计为已处理一项，减少 pending
        entry.pending = Math.max(0, entry.pending - 1);
        this.#logger.info(`Batch ${batchId} progress: ${entry.pending} remaining`);
        if (entry.pending === 0) {
          this.#batchTrack.delete(batchId);
          this.#logger.info(`Batch ${batchId} completed, requesting full PDF list`);
          this.#loadPDFList();
          this.#eventBus.emit(PDF_MANAGEMENT_EVENTS.BATCH.COMPLETED, { batchRequestId: batchId }, { actorId: 'PDFManager' });
        }
      }
    } catch (e) {
  ...
  handleBatchRemove(batchData) {
    ...
    this.#logger.info(`Handling batch remove request for ${files.length} files, timestamp: ${timestamp}`);

    // 为批量删除生成唯一的请求ID并跟踪剩余计数
    const batchRequestId = this.#generateRequestId();
    this.#batchTrack.set(batchRequestId, { pending: files.length, files: [...files] });

    // 按文件逐条发送删除请求，包含批次元数据，兼容后端单文件删除接口
    files.forEach((file, index) => {
      const fileEntry = this.#pdfs.find(p => p.id === file || p.filename === file) || null;
      let filename = fileEntry ? fileEntry.filename : (typeof file === 'string' ? file : undefined);
      if (filename && !filename.endsWith('.pdf')) filename = `${filename}.pdf`;
