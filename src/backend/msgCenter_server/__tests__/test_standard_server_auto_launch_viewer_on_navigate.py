...
@pytest.mark.smoke
def test_navigate_no_target_auto_launches_even_without_sender_socket(tmp_path):
    data_dir = str(tmp_path / "data")
    db_path = str(tmp_path / "test-db.sqlite")
    server = StandardWebSocketServer(data_dir=data_dir, db_path=db_path)

    got = {"open": None, "sock": "unset"}

    def _on_msg(sock, msg):
        if isinstance(msg, dict) and msg.get("type") == "app-window:open:requested":
            got["open"] = msg
            got["sock"] = sock
    ...
    resp = server.handle_message(payload, client_socket=None)
