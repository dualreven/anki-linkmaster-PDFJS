...
from src.backend.pdfFile_server import HttpFileServer
import logging
import json
import socket
import subprocess
...
class AnkiLinkMasterApp:
    ...
    def run(self, module="pdf-viewer", vite_port=3000, pdf_id=None, ws_port=8765, http_port=8080):
        """运行应用

        Args:
            module: 要加载的前端模块 (pdf-home 或 pdf-viewer)
            vite_port: Vite开发服务器端口
            pdf_id: PDF文件ID (仅pdf-viewer模块有效)
        ...
        # 初始化HTTP文件服务器（使用传入的端口，允许从环境变量覆盖）
        http_port_env = os.environ.get("BACKEND_HTTP_PORT")
        actual_http_port = int(http_port_env) if http_port_env and http_port_env.isdigit() else http_port
        self.http_server = HttpFileServer(port=actual_http_port)
        if self.http_server.start():
            logger.info("HTTP文件服务器启动成功")
        else:
            logger.warning("HTTP文件服务器启动失败，文件传输功能受限")
        
        # 初始化并启动WebSocket服务器（使用传入的端口，允许从环境变量覆盖）
        ws_port_env = os.environ.get("BACKEND_WS_PORT")
        actual_ws_port = int(ws_port_env) if ws_port_env and ws_port_env.isdigit() else ws_port
        self.websocket_server = StandardWebSocketServer(host="127.0.0.1", port=actual_ws_port, app=self)
        ...
        # 处理命令行传入的PDF ID
        if self.command_line_handler and self.pdf_id:
            self.command_line_handler.handle_command_line_file(self.pdf_id, module)
