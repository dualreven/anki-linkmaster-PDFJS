...
// Mock Tabulator
const mockTabulatorImplementation = (container, options) => {
  ...
  tabulatorElement.style.position = 'relative'; // 确保 offsetParent 不为 null
...
jest.mock('tabulator-tables', () => {
  return {
    Tabulator: jest.fn().mockImplementation(mockTabulatorImplementation)
  };
});
...
describe('DOM 元素验证', () => {
  ...
  describe('TableWrapper DOM 元素', () => {
    ...
    test('应该正确显示表格元素', () => {
      ...
      // 由于在测试环境中，offsetParent 可能为 null，所以我们只检查元素是否存在于 DOM 中
      expect(document.body.contains(tabulatorElement)).toBe(true);
    });
  ...
  describe('PDFHomeApp DOM 元素', () => {
    ...
    test('应该正确初始化并创建表格元素', async () => {
      ...
      const { ErrorHandler } = await import('../../../../src/frontend/common/error/error-handler.js');
      const WSClient = (await import('../../../../src/frontend/common/ws/ws-client.js')).default;
      const PDFManager = (await import('../../../../src/frontend/common/pdf/pdf-manager.js')).default;
      const { UIManager } = await import('../../../../src/frontend/pdf-home/ui-manager.js');
      
      // 创建 PDFHomeApp 实例
      pdfHomeApp = {
        tableWrapper: null,
        initialize: async function() {
          this._logger = new MockLogger("PDFHomeApp");
          this._eventBus = new EventBus({
            enableValidation: true,
            logLevel: 'DEBUG',
          });
          this._errorHandler = new ErrorHandler(this._eventBus);
          this._websocketManager = WSClient("ws://localhost:8765", this._eventBus);
          this._pdfManager = PDFManager(this._eventBus);
          this._uiManager = new UIManager(this._eventBus);
          ...
          if (tableContainer) {
            ...
            this._uiManager.pdfTable = this.tableWrapper;
          }
          
          await this._pdfManager.initialize();
        ...
        getState: function() {
          return {
            ...
            websocketConnected: this._websocketManager.isConnected(),
            pdfCount: this._pdfManager.getPDFs().length,
          };
        },
  ...
  describe('DOM 元素错误处理', () => {
    ...
    test('应该处理容器不存在的情况', () => {
      ...
      // 创建一个不存在的容器引用
      const nonExistentContainer = document.getElementById('pdf-table-container');
      
      expect(() => {
        new TableWrapper(nonExistentContainer);
      }).toThrow('Container not found');
    });

    test('应该处理无效的容器参数', () => {
      const invalidContainers = [null, undefined];
      ...
      // 对于空字符串和其他无效选择器，TableWrapper 会尝试查询 DOM
      // 这些情况下会抛出不同的错误，所以我们单独测试
      expect(() => {
        new TableWrapper('');
      }).toThrow();
      
      expect(() => {
        new TableWrapper('#non-existent-selector');
      }).toThrow('Container not found');
    });
