...
describe('表格回退机制测试', () => {
  ...
  beforeEach(() => {
    ...
    if (typeof global !== 'undefined') {
      global._forceTabulatorFailure = false;
    }
  });
  ...
  describe('Tabulator 初始化失败时的回退机制', () => {
    ...
    test('回退表格应该支持基本的行选择功能', () => {
      ...
      if (checkboxes.length > 0) {
        // 找到第一个数据行的复选框（跳过全选复选框）
        const dataRowCheckboxes = Array.from(checkboxes).filter(cb => 
          cb.closest('tbody') && cb.closest('tr')
        );
        
        if (dataRowCheckboxes.length > 0) {
          dataRowCheckboxes[0].checked = true;
          
          // 触发 change 事件
          const event = new Event('change', { bubbles: true });
          dataRowCheckboxes[0].dispatchEvent(event);
    ...
    test('回退表格应该支持清空数据功能', () => {
      ...
      if (tableWrapper) {
        ...
        if (rows.length === 1) {
          ...
          const emptyCell = secondRow.querySelector('td');
          expect(emptyCell).not.toBeNull();
          expect(emptyCell.textContent).toContain('暂无数据');
        } else {
          // 其他情况都是失败的
