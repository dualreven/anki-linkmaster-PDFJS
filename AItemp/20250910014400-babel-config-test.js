...
describe('Babel配置优化验证', () => {
  ...
  test('配置文件应该包含正确的插件', () => {
    ...
    expect(babelConfig.plugins.length).toBe(4);
  });
  ...
  test('应该包含新增的有用插件', () => {
    const plugins = babelConfig.plugins;
    expect(plugins).toContain('@babel/plugin-proposal-optional-chaining');
    expect(plugins).toContain('@babel/plugin-proposal-nullish-coalescing-operator');
  });
...
describe('Babel配置功能验证', () => {
  ...
  test('配置应该支持可选链操作符', () => {
    ...
    const hasOptionalChainingPlugin = babelConfig.plugins.includes('@babel/plugin-proposal-optional-chaining');
    expect(hasOptionalChainingPlugin).toBe(true);
  });

  test('配置应该支持空值合并操作符', () => {
    // 这个测试验证空值合并操作符插件是否正确配置
    const hasNullishCoalescingPlugin = babelConfig.plugins.includes('@babel/plugin-proposal-nullish-coalescing-operator');
